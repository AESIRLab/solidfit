...
public class WorkoutItemDaoImpl(
  ...
  override suspend fun insert(item: WorkoutItem) {
    ...
    val mdetailsJson = model.createProperty(Utilities.NS_WorkoutItem + "detailsJson")
    val detailsJsonLiteral = ResourceFactory.createTypedLiteral(item.detailsJson)
    mThingUri.addLiteral(mdetailsJson, detailsJsonLiteral)
  ...
  override suspend fun update(item: WorkoutItem) {
    ...
    if (model.containsResource(resource)) {
      ...
      val mediaUriLiteral = ResourceFactory.createTypedLiteral(item.mediaUri)
      val mdetailsJson = model.createProperty(Utilities.NS_WorkoutItem + "detailsJson")
      resourceInModel.removeAll(mdetailsJson)
      val detailsJsonLiteral = ResourceFactory.createTypedLiteral(item.detailsJson)
      resourceInModel.addProperty(mdetailsJson, detailsJsonLiteral)
